...
//vec2 drawSize = vec2(1.0, 1.0);
uniform float size;

//vec2 texSize = textureSize(uSampler);
//texSize /= inputSize.zw;

void main(void){
    //spritesize / offset = relativer faktor???
    vec2 cutOff = clamp(drawSize/2.0 - drawPosition, vec2(0.0, 0.0), inputSize.xy);
    vec2 position=size * vTextureCoord.xy - (size -1.0) * (0.5*drawSize.xy - cutOff.xy) * inputSize.zw - shift * inputSize.zw;
    vec4 sample = texture2D (uSampler,  position);
...
J=(t,e,r)=>e in t?Lt(t,e,{enumerable:!0,configurable:!0,writable:!0,value:r}):t[e]=r,ee=(t,e)=>{
...
const O=class extends a.Filter{
    constructor(t){
        super(),
        this.angle=45,
        this._drawSize={x:100, y:100},
        this._size=1.2,
        this._distance=5,
        this._resolution=a.settings.FILTER_RESOLUTION;
        const e=t?ee(ee({},O.defaults),t):O.defaults,{kernels:r,blur:i,quality:o,pixelSize:s,resolution:n}=e;
        ...
        this._tintFilter.uniforms.drawSize={x:100, y:100}, //custom by damian
        this._tintFilter.uniforms.drawPosition={x:1000, y:500}, //custom by damian
        this._tintFilter.uniforms.spriteSize={x:100, y:100}, //custom by damian
        this._tintFilter.uniforms.size=this._size,
        this._tintFilter.uniforms.height=100, //custom by damian
        this._tintFilter.resolution=n,this._blurFilter=r?new T(r):new T(i,o),
        this.pixelSize=s,
        this.resolution=n;
const{shadowOnly:l,rotation:u,distance:h,offset:p,alpha:S,color:g}=e;
this.shadowOnly=l,u!==void 0&&h!==void 0?(this.rotation=u,this.distance=h):this.offset=p,this.alpha=S,this.color=g
}
...
apply(t,e,r,i){
    ...
    _updatePadding(){
        //consider size here?
        const t=Math.max(Math.abs(this._offset.x),
        Math.abs(this._offset.y)) * 2;
    ...
    set drawSize(t){
        this._drawSize=t, 
        this._tintFilter.uniforms.drawSize = {x: t.x + 2*this.padding, y: t.y + 2*this.padding}, 
        this._tintFilter.uniforms.spriteSize = {x: t.x, y: t.y}
    }
    get drawSize(){return this._drawSize}
    set drawPosition(t){this._drawPosition=t, this._tintFilter.uniforms.drawPosition = {x: t.x, y: t.y}}
    get drawPosition(){return this._drawPosition}
    set offset(t){this._offset.copyFrom(t),this._updatePadding()}
    get offset(){return this._offset}
    set size(t){this._size=t, this._tintFilter.uniforms.size = t}
    get size(){return this._size}
    get resolution(){return this._resolution}
    set resolution(t){this._resolution=t,this._tintFilter&&(this._tintFilter.resolution=t),this._blurFilter&&(this._blurFilter.resolution=t)}
    get distance(){return this._distance}
    set distance(t){this._distance=t,this._updatePadding(),this._updateShift(), this._tintFilter.uniforms.height = t}
    get rotation(){return this.angle/a.DEG_TO_RAD}
    set rotation(t){a.utils.deprecation("5.3.0","DropShadowFilter rotation is deprecated, use offset"),this.angle=t*a.DEG_TO_RAD,this._updateShift()}
    get alpha(){return this._tintFilter.uniforms.alpha}
    set alpha(t){this._tintFilter.uniforms.alpha=t}
    get color(){return a.utils.rgb2hex(this._tintFilter.uniforms.color)}
    set color(t){a.utils.hex2rgb(t,this._tintFilter.uniforms.color)}
    get kernels(){return this._blurFilter.kernels}
    set kernels(t){this._blurFilter.kernels=t}
    get blur(){return this._blurFilter.blur}
    set blur(t){this._blurFilter.blur=t,this._updatePadding()}
    get quality(){return this._blurFilter.quality}
    set quality(t){this._blurFilter.quality=t}
    get pixelSize(){return this._blurFilter.pixelSize}
    set pixelSize(t){this._blurFilter.pixelSize=t}
};
let te=O;
te.defaults={offset:{x:4,y:4},color:0,alpha:.5,shadowOnly:!1,kernels:null,blur:2,quality:3,pixelSize:1,resolution:a.settings.FILTER_RESOLUTION};
